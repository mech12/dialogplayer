package com.mp3sdk.common.constants;

import com.ytsdk.mp3lib.MP3SDK;

public class Constants {

	public static final boolean STARTAPP_FULL_SCREEN_ADS = false;
	public static final boolean STARTAPP_BANNER_ADS = false;
	public static final boolean ADMOB_BANNER_ADS = true;
	public static final boolean ADMOB_FULLSCREEN_ADS = true;

	public static final int MP3SDK_ENGINE = MP3SDK.MP3_ENGINE_SOUND_BOWL;
	// public static final int MP3SDK_ENGINE = MP3SDK.MP3_ENGINE_GO_EAR;
	// public static final int MP3SDK_ENGINE = MP3SDK.MP3_ENGINE_MP3_SKULL;

	public static final int NUM_ADS_PER_SEARCH = 3;

}
